...
/**
  ...
  * @param dbPath
  */
class WatchedReaderFlow(readerId: String, dbPath: Path) extends GraphStage[FlowShape[Unit, CommitableResult]] {

  val out: Outlet[CommitableResult] = Outlet("CommitableResultSource.out")
  val in: Inlet[Unit] = Inlet("WriterNotifications.in")

  override def shape: FlowShape[Unit, CommitableResult] = FlowShape(in, out)
